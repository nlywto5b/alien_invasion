...
# screen
def check_fps_time(flut):
    '''检查和上一次更新fps时间是否相差1秒'''
    return int(round(time() - flut, 5)) == 1
...
def write_hs(ai_settings, stats):
    '''写入最高分'''
    if ai_settings.difficulty == 1:
        ai_settings.score["easy_score"] = stats.high_score
    elif ai_settings.difficulty == 2:
        ai_settings.score["normal_score"] = stats.high_score
    elif ai_settings.difficulty == 3:
        ai_settings.score["difficult_score"] = stats.high_score
    with open('score.pickle', 'wb') as f:
        dump(ai_settings.score, f)
...
def init_score_file():
    '''初始化分数文件'''
    with open('score.pickle', 'wb') as f:
        dump({'easy_score': 0,'normal_score': 0,'difficult_score': 0}, f)
    with open('backup.pickle', 'wb') as f:
        dump({'easy_score': 0,'normal_score': 0,'difficult_score': 0}, f)
...
def get_score():
    ...
    try:
        ...
        init_score_file()
        return {'easy_score': 0,'normal_score': 0,'difficult_score': 0}
    except:  # 文件损坏
        try:
            with open('backup.pickle', 'rb') as bk:  # 尝试使用备份
                temp = load(bk)
            with open('score.pickle', 'wb') as f:  # 覆写分数文件
                dump(temp, f)
        except :  # 分数和备份文件均损坏
            tk.messagebox.showerror('出现错误', '分数信息存储文件和备份均已损坏, \
因此, 你以前的记录全部丢失. 确认后将自动退出程序, 重新启动程序即可正常, 但分数清零')
            init_score_file()            
            exit()  # 退出
